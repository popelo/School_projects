...
*/
	...
	s->top = 0; //inicializácia zásobníka
}
...
*/
	...
	if( stackEmpty( s ) != 0 ) //zásobník je prázdny
		{
		    stackError( SERR_TOP );
		    return;
		}
	
	*c = s->arr[s->top]; // Priradí hodnotu z vrcholu do c
...
*/
	...
	if( stackEmpty( s ) != 0 ) //prázdny zásobník
		return;

	s->top -= 1; //posuniem zásobník(odstranim prvok)	
}
...
*/
	...
	if ( stackFull( s ) != 0 ) //zásobník je plný
		{
			stackError( SERR_PUSH );
			return;
		}
 
	s->top += 1; //posuniem zásobník
	s->arr[s->top] = c; //pridelim hodnotu
